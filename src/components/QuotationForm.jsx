...
function translateNotesToEN(html) {
    ...
    res = res.replace(/กรณีที่ลูกค้าตกลงโอนชำระเงินรายการตามใบเสนอราคาเรียบร้อยแล้ว/g, 'If the customer has transferred payment according to the quotation');
...
export default function QuotationForm({ editId, onBack, onSave, viewOnly, isHistory, hideControls }) {
    ...
    // Removed duplicate customerList and modal state
    ...
    // Ensure customerId is populated for editing if it was not set (since backend Quotation doesn't store CustomerID)
    useEffect(() => {
        ...
        setItems(prev => prev.map(item => {
            if (item.id === id) {
                ...
                if (field === 'manualTotal') {
                    ...
                    // reverse-calculate price to keep qty*price consistent, using 4 decimals to avoid rounding errors
    ...
    // For FDA, always calculate 7% VAT and show it.
                        ...
                        <div className="form-row" style={{ backgroundColor: '#fce4ec', padding: '15px', borderRadius: '12px', marginBottom: '20px', border: '1px solid #f8bbd0' }}>
                            <div className="form-group" style={{ marginBottom: 0 }}>
                                ...
                                <CustomSelect name="docType" value={formData.docType} onChange={handleFormChange} required>
                                    ...
                                    <option value="quotation_fda_thc">ใบเสนอราคา อย. (FDA Quotation) - THC</option>
                    ...
                    <div className="q-section q-section--customer">
                        ...
                        {formData.docType && formData.docType.includes('fda') && (
                            <div style={{ backgroundColor: '#e8f5e9', padding: '15px', borderRadius: '8px', border: '1px solid #a5d6a7', marginTop: '20px' }}>
                                <h4 style={{ margin: '0 0 15px 0', color: '#2e7d32', fontSize: '15px', fontWeight: 'bold' }}>ข้อมูลเฉพาะใบเสนอราคา อย. (FDA)</h4>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style={{ width: '50%', textAlign: 'right', border: 'none', verticalAlign: 'bottom' }}>
                                                {isFda && <div style={{ color: 'red', fontSize: '11pt', fontWeight: 'bold' }}>{isEn ? '** For FDA Quotation Only **' : '** ข้อมูลเฉพาะใบเสนอราคา อย. (FDA) **'}</div>}
                                                <div style={{ fontSize: '16pt', fontWeight: 'bold' }}>{isEn ? 'Quotation' : 'ใบเสนอราคา'}</div>
                                                {!isEn && <div style={{ fontSize: '14pt', fontWeight: 'bold' }}>Quotation</div>}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
