...
function translateNotesToEN(html) {
    ...
    res = res.replace(/กรณีที่ลูกค้าตกลงโอนชำระเงินรายการตามใบวางบิล\/ใบส่งสินค้าเรียบร้อยแล้ว/g, 'If the customer has transferred payment according to the billing invoice');
...
const DEFAULT_NORMAL_NOTES = `<p><strong><span style="font-size: 12px;">เงื่อนไข & ข้อตกลง :</span></strong></p>
<p><span style="font-size: 12px;">&nbsp;&nbsp;&nbsp;&nbsp;1. สินค้าที่ผลิตไม่สามารถเปลี่ยนแปลง ยกเลิก หรือคืนในกรณีการสั่งผลิตสินค้า ยกเว้นสินค้ามีปัญหาจากกระบวนการผลิต</span></p>
<p><span style="font-size: 12px;">&nbsp;&nbsp;&nbsp;&nbsp;2. การตรวจรับสินค้าให้ตรวจสอบหลังรับสินค้า ต้องรายงานภายในวันเท่านั้น มิฉะนั้นถือว่ายอมรับสินค้าที่ส่งมอบ</span></p>
<p><span style="font-size: 12px;">&nbsp;&nbsp;&nbsp;&nbsp;3. สินค้าที่มอบแล้วจะไม่สามารถเปลี่ยนแปลงใดๆ ขอสงวนสิทธิ์เรียกชำระเงินตามมูลค่าสินค้าที่ส่งมอบ</span></p>`;
...
export default function DeliveryOrderForm({ editId, onBack, onSave, viewOnly, isHistory }) {
    ...
    const { signatures: availableSignatures, userSignatures, bossSignatures, getSignatureUrl } = useSignatures();
    ...
    useEffect(() => {
        if (!editId) {
            // ใบส่งสินค้า (DO) ลายเซ็นผู้มีอำนาจลงนามต้องเป็นเจ้านาย (ธวัช จรุงพิรวงศ์)
        ...
        if (editId) {
            const fetchQuotation = async () => {
                try {
                    ...
                    const endpoint = isHistoryEdit ? `/delivery-orders/history/${targetId}` : `/delivery-orders/${targetId}`;
    ...
    const handleSave = async (e) => {
        ...
        const payload = {
            deliveryOrderNo: formData.billNo,
        ...
        try {
            const url = editId ? `${API_BASE}/delivery-orders/${editId}` : `${API_BASE}/delivery-orders`;
            ...
            if (json.success) {
                setStatus('success');
                showAlert('สำเร็จ', 'บันทึกข้อมูลใบส่งสินค้า DELIVERY ORDERเรียบร้อยแล้ว', 'success');
                setTimeout(() => {
                    if (onSave) onSave();
        ...
        <div className="q-form-wrapper">
                ...
                <>
                    ...
                    <div className="view-only-controls" style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginBottom: '20px', padding: '10px', background: '#f8fafc', borderRadius: '8px' }}>
                        ...
                        <button className="btn-primary" type="button" onClick={handlePrint}>
                            <Printer size={16} /> พิมพ์ใบส่งสินค้า DELIVERY ORDER
                        </button>
                    </div>
            ...
            <div className="q-container" style={{ display: viewOnly ? 'none' : 'block' }}>
                ...
                <div className="q-header">
                    <h1><FileText size={22} color="#4f46e5" /> ฟอร์มออกใบส่งสินค้า DELIVERY ORDER</h1>
                    <p>กรุณากรอกข้อมูลให้ครบถ้วนเพื่อออกใบส่งสินค้า DELIVERY ORDERและเชื่อมต่อกับ Sales Order</p>
                </div>
                ...
                <form onSubmit={handleSave}>
                    ...
                    <div className="q-section q-section--customer">
                        ...
                        {formData.docType && formData.docType.includes('fda') && (
                            <div style={{ backgroundColor: '#e8f5e9', padding: '15px', borderRadius: '8px', border: '1px solid #a5d6a7', marginTop: '20px' }}>
                                <h4 style={{ margin: '0 0 15px 0', color: '#2e7d32', fontSize: '15px', fontWeight: 'bold' }}>ข้อมูลเฉพาะใบส่งสินค้า DELIVERY ORDER อย. (FDA)</h4>
                    ...
                    {/* ===== Section: ข้อมูลเพิ่มเติมเฉพาะใบกำกับภาษี/ใบส่งสินค้า ===== */}
                    <div className="q-section" style={{ backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0' }}>
                        <div className="q-section-header">
                            ...
                            <div>
                                <div className="q-section-title" style={{ color: '#166534' }}>ข้อมูลเพิ่มเติมเฉพาะใบกำกับภาษี/ใบส่งสินค้า</div>
                                <div className="q-section-desc" style={{ color: '#15803d' }}>ข้อมูลอ้างอิง สถานที่ส่ง ติดต่อ และเงื่อนไขการชำระเงิน</div>
                            </div>
                        </div>
                        
                        <div className="q-form-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px 20px', marginTop: '15px' }}>
                            <div className="form-group">
                            ...
                            <div className="form-group">
                                ...
                                <select name="termOfPayment" value={formData.termOfPayment} onChange={handleFormChange} style={{ width: '100%', padding: '10px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '15px', outline: 'none', backgroundColor: '#fff' }}>
                                    ...
                                    <option value="30 วัน">เครดิต 30 วัน</option>
            ...
            <div id="q-print-container">
                {isFda ? (
                    <div style={{ padding: '20px', fontFamily: "'Sarabun', sans-serif" }}>
                        ...
                        {/* Extra Document Info Table */}
                        {(formData.purchaseNo || formData.termOfPayment || formData.customerOrder || formData.salesperson || formData.contactPerson) && (
                            <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid black', borderTop: 'none', fontSize: '11pt', marginBottom: '-1px', tableLayout: 'fixed' }}>
                                <tbody>
                                    <tr style={{ backgroundColor: 'white' }}>
                                        <td style={{ width: '20%', border: '1px solid black', borderTop: 'none', padding: '4px', fontWeight: 'bold', borderLeft: 'none' }}>
                                            ใบสั่งซื้อ <br/><span style={{ fontWeight: 'normal', fontSize: '8pt' }}>P/O No.</span>
                                        </td>
                                        <td style={{ width: '20%', border: '1px solid black', borderTop: 'none', padding: '4px', fontWeight: 'bold' }}>
                                            ผู้ติดต่อ <br/><span style={{ fontWeight: 'normal', fontSize: '8pt' }}>Contact By</span>
                                        </td>
                                        <td style={{ width: '20%', border: '1px solid black', borderTop: 'none', padding: '4px', fontWeight: 'bold' }}>
                                            ใบสั่งซื้อลูกค้า <br/><span style={{ fontWeight: 'normal', fontSize: '8pt' }}>Customer Order</span>
                                        </td>
                                        <td style={{ width: '20%', border: '1px solid black', borderTop: 'none', padding: '4px', fontWeight: 'bold' }}>
                                            เงื่อนไขการชำระเงิน <br/><span style={{ fontWeight: 'normal', fontSize: '8pt' }}>Term Of Payment</span>
                                        </td>
                                        <td style={{ width: '20%', border: '1px solid black', borderTop: 'none', padding: '4px', fontWeight: 'bold', borderRight: 'none' }}>
                                            พนักงานขาย <br/><span style={{ fontWeight: 'normal', fontSize: '8pt' }}>Salesperson</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style={{ border: '1px solid black', borderLeft: 'none', borderBottom: 'none', padding: '6px' }}>{formData.purchaseNo || '\u00A0'}</td>
                                        <td style={{ border: '1px solid black', borderBottom: 'none', padding: '6px' }}>{formData.contactPerson || '\u00A0'}</td>
                                        <td style={{ border: '1px solid black', borderBottom: 'none', padding: '6px' }}>{formData.customerOrder || '\u00A0'}</td>
                                        <td style={{ border: '1px solid black', borderBottom: 'none', padding: '6px' }}>{formData.termOfPayment || '\u00A0'}</td>
                                        <td style={{ border: '1px solid black', borderRight: 'none', borderBottom: 'none', padding: '6px' }}>{formData.salesperson || '\u00A0'}</td>
                                    </tr>
                                </tbody>
                            </table>
                        )}
                            ...
                            <tbody>
                                ...
                                {items.filter(item => item.name && item.name.trim() !== '').map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td colSpan="2" style={{ border: '1px solid black', padding: '4px 8px', textAlign: 'center' }}>
                                            {item.image ? (
                                                <img src={item.image} alt={item.name} style={{ width: '60px', height: '60px', objectFit: 'contain', display: 'inline-block' }} />
                                            ) : '-'}
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td style={{ width: '50%', padding: '2px 8px', verticalAlign: 'middle', border: 'none' }}>
                                ...
                                {compAddr2 && <div style={{ fontSize: '8pt' }}>{compAddr2}</div>}
                                <div style={{ fontSize: '9pt', marginTop: '1px' }}>
                                    {compTax}
                                </div>
                            </td>
                            <td style={{ width: '35%', textAlign: 'center', verticalAlign: 'top', border: 'none', padding: '10px' }}>
                                {isFda && <div style={{ color: 'red', fontSize: '11pt', fontWeight: 'bold', marginBottom: '8px' }}>{isEn ? '** For FDA Billing Invoice Only **' : '** ข้อมูลเฉพาะใบส่งสินค้า DELIVERY ORDER อย. (FDA) **'}</div>}
                                <div style={{ backgroundColor: '#2ecc71', color: 'white', borderRadius: '25px', padding: '8px 5px', position: 'relative' }}>
                                    <div style={{ fontSize: '15pt', fontWeight: 'bold', whiteSpace: 'nowrap' }}>
                                        {isEn ? 'DELIVERY ORDER' : 'ใบส่งสินค้า DELIVERY ORDER'}
                                    </div>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10pt' }}>
                            <tbody>
                                <tr style={{ backgroundColor: '#d5f5e3' }}>
                                    ...
                                    <td style={{ width: '60%', padding: '4px 8px', borderBottom: '1px solid #1a7a3a', fontWeight: 'bold' }}>
                                        {formData.billNo || '-'}
                                    </td>
                                </tr>
                                <tr>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                        <thead>
                            <tr style={{ backgroundColor: '#d5f5e3' }}>
                                ...
                                <th style={{ width: '13%', borderRight: '1px solid #1a7a3a', borderBottom: '1px solid #1a7a3a', padding: '4px 2px', fontSize: '9pt' }}>
                                    ราคาต่อหน่วย<br /><span style={{ fontWeight: 'normal', fontSize: '8pt' }}>Unit Price</span>
                                </th>
                                <th style={{ width: '10%', borderRight: '1px solid #1a7a3a', borderBottom: '1px solid #1a7a3a', padding: '4px 2px', fontSize: '9pt' }}>
                                    ส่วนลด<br /><span style={{ fontWeight: 'normal', fontSize: '8pt' }}>Discount</span>
                                </th>
                                <th style={{ width: '13%', borderBottom: '1px solid #1a7a3a', padding: '4px 2px', fontSize: '9pt' }}>
                ...
                {/* Signatures for Delivery Order (2 separated boxes - Flex 6 and Flex 4) */}
