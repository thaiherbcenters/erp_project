        ...
        if (!editId) {
            ...
            const fetchNextNo = async () => {
                try {
                    ...
                    const res = await fetch(`${API_BASE}/tax-invoices/next-number?docType=${formData.docType}${dateParam}`);
                            ...
                            <tbody>
                                ...
                                {items.filter(item => item.name && item.name.trim() !== '').map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td colSpan="2" style={{ border: '1px solid black', padding: '4px 8px', textAlign: 'center' }}>
                                            {item.image ? (
                                                <img src={item.image} alt={item.name} style={{ width: '60px', height: '60px', objectFit: 'contain', display: 'block', margin: '0 auto' }} />
                                            ) : '-'}
                                        </td>
                                    </tr>
                                ))}
