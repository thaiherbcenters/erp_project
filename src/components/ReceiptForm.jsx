...
function translateNotesToEN(html) {
    ...
    res = res.replace(/กรณีที่ลูกค้าตกลงโอนชำระเงินรายการตามใบวางบิล\/ใบเสร็จรับเงินเรียบร้อยแล้ว/g, 'If the customer has transferred payment according to the billing invoice');
...
export default function ReceiptForm({ editId, onBack, onSave, viewOnly, isHistory, initialFromQuotation }) {
        ...
        if (!editId) {
            ...
            const fetchNextNo = async () => {
                try {
                    ...
                    const res = await fetch(`${API_BASE}/receipts/next-number?docType=${formData.docType}${dateParam}`);
        ...
        if (editId) {
            const fetchQuotation = async () => {
                try {
                    ...
                    const endpoint = isHistoryEdit ? `/receipts/history/${targetId}` : `/receipts/${targetId}`;
    ...
    const handleSave = async (e) => {
        ...
        try {
            const url = editId ? `${API_BASE}/receipts/${editId}` : `${API_BASE}/receipts`;
            ...
            if (json.success) {
                setStatus('success');
                showAlert('สำเร็จ', 'บันทึกข้อมูลใบเสร็จรับเงิน RECEIPT (ORIGINAL)เรียบร้อยแล้ว', 'success');
                setTimeout(() => {
                    if (onSave) onSave();
        ...
        <div className="q-form-wrapper">
                ...
                <>
                    ...
                    <div className="view-only-controls" style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginBottom: '20px', padding: '10px', background: '#f8fafc', borderRadius: '8px' }}>
                        ...
                        <button className="btn-primary" type="button" onClick={handlePrint}>
                            <Printer size={16} /> พิมพ์ใบเสร็จรับเงิน RECEIPT (ORIGINAL)
                        </button>
                    </div>
            ...
            <div className="q-container" style={{ display: viewOnly ? 'none' : 'block' }}>
                ...
                <div className="q-header">
                    <h1><FileText size={22} color="#4f46e5" /> ฟอร์มออกใบเสร็จรับเงิน RECEIPT (ORIGINAL)</h1>
                    <p>กรุณากรอกข้อมูลให้ครบถ้วนเพื่อออกใบเสร็จรับเงิน RECEIPT (ORIGINAL)และเชื่อมต่อกับ Sales Order</p>
                </div>
                        ...
                        <div className="form-row" style={{ backgroundColor: '#fce4ec', padding: '15px', borderRadius: '12px', marginBottom: '20px', border: '1px solid #f8bbd0', boxSizing: 'border-box' }}>
                            <div className="form-group" style={{ marginBottom: 0, minWidth: 0 }}>
                                ...
                                <CustomSelect name="docType" value={formData.docType} onChange={handleFormChange} required>
                                    ...
                                    <option value="delivery_order_psf">ใบเสร็จรับเงิน RECEIPT (ORIGINAL) - PSF</option>
                    ...
                    <div className="q-section q-section--customer">
                        ...
                        {formData.docType && formData.docType.includes('fda') && (
                            <div style={{ backgroundColor: '#e8f5e9', padding: '15px', borderRadius: '8px', border: '1px solid #a5d6a7', marginTop: '20px' }}>
                                <h4 style={{ margin: '0 0 15px 0', color: '#2e7d32', fontSize: '15px', fontWeight: 'bold' }}>ข้อมูลเฉพาะใบเสร็จรับเงิน RECEIPT (ORIGINAL) อย. (FDA)</h4>
