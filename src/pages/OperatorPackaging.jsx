/**
 * =============================================================================
 * Packaging.jsx — หน้า งานบรรจุ (ฝ่ายผลิต)
 * =============================================================================
...
export default function Packaging() {
    ...
    const getRequiredMaterials = (order) => {
        ...
        const formula = MOCK_FORMULAS?.find(f => f.name === order.formulaName || f.id === order.formulaName || f.name === order.product || f.id === order.product);
            ...
            <div className="pkg-modal-overlay" onClick={() => setSelectedOrder(null)}>
                <div className="pkg-modal" style={{ maxWidth: 640 }} onClick={e => e.stopPropagation()}>
                    ...
                    <div style={{ padding: '16px 24px', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                        ...
                        {o.status === 'QC ผ่าน' && (
                            <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#10b981', fontWeight: 600, fontSize: 13, background: '#ecfdf5', padding: '8px 14px', borderRadius: 8 }}>
                                <CheckCircle2 size={14} /> ผลิตและตรวจสอบสำเร็จ
                            </span>
                        )}
                                                ...
                                                <td style={{ whiteSpace: 'nowrap' }}>
                                                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                                        <button 
                                                            ...
                                                            title="ดูรายละเอียดคำสั่งบรรจุ"
                                                        >
                                                            <Eye size={16} />
                                                        </button>
                                                        
                                                        {order.requisitionJSON && (
                                                            <button 
                                                                onClick={async (e) => {
                                                                    e.stopPropagation();
                                                                    try {
                                                                        const res = await fetch(`${API_URL}/print/requisition/${order.id}`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }});
                                                                        if (!res.ok) throw new Error('Failed to fetch pdf');
                                                                        const blob = await res.blob();
                                                                        const url = window.URL.createObjectURL(blob);
                                                                        window.open(url, '_blank');
                                                                ...
                                                                title="ดูใบเบิกวัสดุบรรจุภัณฑ์ (PDF)"
                                                            >
                                                                <FileText size={16} />
                                                            </button>
                                                        )}

                                                        {['รอ QC Final', 'QC ผ่าน', 'ส่งมอบแล้ว'].includes(order.status) && (
                                                            <button 
                                                                onClick={async (e) => {
                                                                    e.stopPropagation();
                                                                    try {
                                                                        const res = await fetch(`${API_URL}/print/qc-request/${order.id}`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }});
    ...
    const getPageTitle = () => {
        switch (currentTab) {
            case 'packaging_main': return 'งานบรรจุ (ฝ่ายผลิต)';
