...
async function createTables() {
    ...
    // ========================================
    console.log('[1/2] Creating Documents table...');
    await pool.request().query(`
        ...
        CREATE TABLE Documents (
            ...
            doc_code       NVARCHAR(50)  NOT NULL UNIQUE,
            doc_name       NVARCHAR(255) NOT NULL,
            category       NVARCHAR(50)  NOT NULL,
            type           NVARCHAR(50)  NOT NULL,
            type_tag       NVARCHAR(50)  NOT NULL,
            revision       NVARCHAR(10)  NOT NULL,
            file_path      NVARCHAR(MAX) NULL,
            standard       NVARCHAR(255) NULL,
            status         NVARCHAR(50)  NOT NULL DEFAULT 'ใช้งาน',
    ...
    // 2. Insert seed data
    // ========================================
    console.log('[2/2] Inserting seed data...');

    let insertedCount = 0;
    for (const doc of DOCUMENTS) {
        try {
            // For testing: Set dummy file paths
            let filePath = null;
            if (doc.id === 'SOP-IT-03') {
                filePath = 'D:\\ERP_Data\\Documents\\SOP-IT-03_dummy.docx';
            } else if (doc.typeTag === 'SOP') {
                filePath = `D:\\ERP_Data\\Documents\\${doc.id}_dummy.pdf`;
            } else if (doc.typeTag === 'Form') {
                filePath = `D:\\ERP_Data\\Documents\\${doc.id}_dummy.xlsx`;
            } else {
                filePath = `D:\\ERP_Data\\Documents\\${doc.id}_dummy.docx`;
            }

            await pool.request()
                .input('doc_code', sql.NVarChar, doc.id)
                .input('doc_name', sql.NVarChar, doc.name)
                .input('category', sql.NVarChar, doc.category)
                .input('type', sql.NVarChar, doc.type)
                .input('type_tag', sql.NVarChar, doc.typeTag)
                .input('revision', sql.NVarChar, doc.revision)
                .input('file_path', sql.NVarChar, filePath)
                .input('standard', sql.NVarChar, doc.standard)
                .input('status', sql.NVarChar, doc.status)
                .input('effective_date', sql.Date, new Date(doc.date))
                .query(`
                    IF NOT EXISTS (SELECT 1 FROM Documents WHERE doc_code = @doc_code)
                    BEGIN
                        INSERT INTO Documents (doc_code, doc_name, category, type, type_tag, revision, file_path, standard, status, effective_date)
                        VALUES (@doc_code, @doc_name, @category, @type, @type_tag, @revision, @file_path, @standard, @status, @effective_date);
                    END
                `);
            insertedCount++;
        } catch (e) {
            console.log(`   ⚠️  ${doc.id}: ${e.message}`);
        }
    }
    console.log(`   ✅ ${insertedCount} documents processed`);

    // --- Check results ---
    console.log('\n-------------------------------------------');
    console.log('📋 Data in Documents table:');
    const result = await pool.request().query(
        'SELECT TOP 5 doc_id, doc_code, doc_name, file_path, standard FROM Documents'
    );
    console.table(result.recordset);

    const countDocs = await pool.request().query('SELECT COUNT(*) AS total FROM Documents');
    console.log(`\n📋 Documents: ${countDocs.recordset[0].total} records`);
