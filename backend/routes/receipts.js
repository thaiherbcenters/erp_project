...
router.get('/status/approved', async (req, res) => {
    try {
        ...
        const result = await pool.request().query(`
            SELECT 
                CustomerID, ReceiptID, ReceiptNo, ContractID, CustomerName, BillDate, 
                GrandTotal, Status
            FROM Receipt
...
router.get('/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch receipt details', error: err.message });
    }
});

// 3. Create new receipt
router.post('/', authorizeRoles('admin', 'sales'), validate(createReceiptSchema), async (req, res) => {
    const { 
        customerId, receiptNo, docType, bankAccount, customerTypeId, customerName, contactPerson, email, address, phone, taxId, 
    ...
    try {
        ...
        const receiptId = headerResult.recordset[0].ReceiptID;
        ...
        if (items && items.length > 0) {
            for (let i = 0; i < items.length; i++) {
                ...
                await itemReq.query(`
                    INSERT INTO ReceiptItem (ReceiptID, ItemOrder, ItemName, Qty, Price, Amount, IsPromo, PromoMultiplier, ImageURL)
            ...
            try {
                ...
                if (custCheck.recordset.length === 0) {
                    ...
                    await pool.request()
                        ...
                        .input('source', sql.NVarChar, 'receipt')
                        .query(`
        ...
        res.status(500).json({ success: false, message: 'Failed to create receipt: ' + err.message, error: err.message });
    }
});

// 4. Update existing receipt
router.put('/:id', authorizeRoles('admin', 'sales'), validate(createReceiptSchema), async (req, res) => {
    const qid = req.params.id;
    const { 
        customerId, receiptNo, docType, bankAccount, customerName, address, phone, taxId, 
        billDate, validUntil, subTotal, discountPercent, discountAmount, 
        afterDiscount, vatRate, vatAmount, shippingCost, grandTotal, 
        depositPercent, depositAmount, remainingAmount, signer, 
        customerOrder, purchaseNo, salesperson, termOfPayment, notes, 
    ...
    try {
        ...
        const backupResult = await backupReq.query(`
            INSERT INTO ReceiptHistory (
                CustomerID, ReceiptID, Revision, ReceiptNo, DocType, BankAccount, CustomerName, Address, Phone, TaxID,
                BillDate, ValidUntil, SubTotal, DiscountPercent, DiscountAmount, AfterDiscount,
                VatRate, VatAmount, ShippingCost, GrandTotal, DepositPercent, DepositAmount,
        ...
        await backupItemsReq.query(`
            INSERT INTO ReceiptItemHistory (HistoryID, ItemOrder, ItemName, Qty, Price, Amount, IsPromo, PromoMultiplier, ImageURL)
        ...
        await request.query(`
            UPDATE Receipt SET
                CustomerID = @customerId, ReceiptNo = @receiptNo, ContractID = @contractId, DocType = @docType, BankAccount = @bankAccount,
        ...
        await delReq.query(`DELETE FROM ReceiptItem WHERE ReceiptID = @id`);
        ...
        res.status(500).json({ success: false, message: 'Failed to update receipt: ' + err.message, error: err.message });
    }
});
...
router.patch('/:id/status', authorizeRoles('admin', 'sales'), validate(updateStatusSchema), async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query(`UPDATE Receipt SET Status = @status, UpdatedAt = GETDATE() WHERE ReceiptID = @id`);
...
router.delete('/:id', authorizeRoles('admin', 'sales'), async (req, res) => {
    try {
        ...
        const existing = await pool.request()
            .input('id', sql.Int, req.params.id)
            .query('SELECT ReceiptNo, CustomerName, GrandTotal FROM Receipt WHERE ReceiptID = @id');
        ...
        await logAction(req, 'DELETE', 'billing-invoices', req.params.id, 
            `ลบใบเสนอราคา ${deleted?.ReceiptNo || req.params.id} — ลูกค้า: ${deleted?.CustomerName}`,
            deleted, null);

        res.json({ success: true, message: 'Receipt deleted successfully' });
    } catch (err) {
        console.error('Error deleting receipt:', err);
        res.status(500).json({ success: false, message: 'Failed to delete receipt', error: err.message });
    }
});

// 7. Get History List for a Receipt
...
router.get('/history/:historyId', async (req, res) => {
    try {
        ...
        const headerResult = await pool.request()
            ...
            .query(`SELECT * FROM ReceiptHistory WHERE HistoryID = @hid`);
        ...
        const itemsResult = await pool.request()
            .input('hid', sql.Int, req.params.historyId)
            .query(`SELECT * FROM ReceiptItemHistory WHERE HistoryID = @hid ORDER BY ItemOrder ASC`);
