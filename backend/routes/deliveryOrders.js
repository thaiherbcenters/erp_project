...
router.get('/', async (req, res) => {
    try {
        ...
        whereClauses.push('(d.CompanyID = @companyId OR (d.CompanyID IS NULL AND @companyId = 1))');
...
router.get('/status/approved', async (req, res) => {
    try {
        ...
        const result = await pool.request().query(`
            SELECT 
                CustomerID, DeliveryOrderID, DeliveryOrderNo, ContractID, CustomerName, BillDate, 
                GrandTotal, Status
            FROM DeliveryOrder
...
// 2. Get single deliveryOrder by ID (with items)
router.get('/:id', async (req, res) => {
    try {
        ...
        if (headerResult.recordset.length === 0) {
            return res.status(404).json({ success: false, message: 'DeliveryOrder not found' });
        }
        
        // Get Items
        const itemsResult = await pool.request()
            .input('id', sql.Int, req.params.id)
            .query(`SELECT * FROM DeliveryOrderItem WHERE DeliveryOrderID = @id ORDER BY ItemOrder ASC`);
        ...
        console.error('Error fetching deliveryOrder details:', err);
        res.status(500).json({ success: false, message: 'Failed to fetch deliveryOrder details', error: err.message });
    }
});

// 3. Create new deliveryOrder
router.post('/', authorizeRoles('admin', 'sales'), validate(createDeliveryOrderSchema), async (req, res) => {
    const { 
        customerId, deliveryOrderNo, docType, bankAccount, customerTypeId, customerName, contactPerson, email, address, phone, taxId, 
    ...
    try {
        ...
        const headerResult = await request.query(`
            INSERT INTO DeliveryOrder (
                CustomerID, DeliveryOrderNo, ContractID, DocType, BankAccount, CustomerName, Address, Phone, TaxID,
        ...
        const deliveryOrderId = headerResult.recordset[0].DeliveryOrderID;
        ...
        if (items && items.length > 0) {
            for (let i = 0; i < items.length; i++) {
                ...
                itemReq.input('qid', sql.Int, deliveryOrderId);
                ...
                await itemReq.query(`
                    INSERT INTO DeliveryOrderItem (DeliveryOrderID, ItemOrder, ItemName, Qty, Price, Amount, IsPromo, PromoMultiplier, ImageURL)
            ...
            try {
                ...
                if (custCheck.recordset.length === 0) {
                    ...
                    await pool.request()
                        ...
                        .input('source', sql.NVarChar, 'deliveryOrder')
                        .query(`
        ...
        await logAction(req, 'CREATE', 'billing-invoices', deliveryOrderId, 
            `สร้างใบเสนอราคา ${finalDeliveryOrderNo} — ลูกค้า: ${customerName} — ยอดรวม: ${grandTotal}`);

        res.status(201).json({ success: true, message: 'DeliveryOrder created successfully', deliveryOrderId });

    } catch (err) {
        if (transaction) await transaction.rollback();
        console.error('Error creating deliveryOrder:', err);
        res.status(500).json({ success: false, message: 'Failed to create deliveryOrder: ' + err.message, error: err.message });
    }
});

// 4. Update existing deliveryOrder
router.put('/:id', authorizeRoles('admin', 'sales'), validate(createDeliveryOrderSchema), async (req, res) => {
    const qid = req.params.id;
    const { 
        customerId, deliveryOrderNo, docType, bankAccount, customerName, address, phone, taxId, 
        billDate, validUntil, subTotal, discountPercent, discountAmount, 
        afterDiscount, vatRate, vatAmount, shippingCost, grandTotal, 
        depositPercent, depositAmount, remainingAmount, signer, 
        customerOrder, purchaseNo, salesperson, termOfPayment, notes, 
    ...
    try {
        ...
        request.input('deliveryOrderNo', sql.NVarChar, deliveryOrderNo);
        request.input('customerId', sql.Int, customerId || null);
        request.input('docType', sql.NVarChar, docType);
        request.input('bankAccount', sql.NVarChar, bankAccount);
        request.input('customerName', sql.NVarChar, customerName);
        ...
        const backupResult = await backupReq.query(`
            INSERT INTO DeliveryOrderHistory (
                CustomerID, DeliveryOrderID, Revision, DeliveryOrderNo, DocType, BankAccount, CustomerName, Address, Phone, TaxID,
                BillDate, ValidUntil, SubTotal, DiscountPercent, DiscountAmount, AfterDiscount,
                VatRate, VatAmount, ShippingCost, GrandTotal, DepositPercent, DepositAmount,
        ...
        await backupItemsReq.query(`
            INSERT INTO DeliveryOrderItemHistory (HistoryID, ItemOrder, ItemName, Qty, Price, Amount, IsPromo, PromoMultiplier, ImageURL)
        ...
        await request.query(`
            UPDATE DeliveryOrder SET
                CustomerID = @customerId, DeliveryOrderNo = @deliveryOrderNo, ContractID = @contractId, DocType = @docType, BankAccount = @bankAccount,
        ...
        await delReq.query(`DELETE FROM DeliveryOrderItem WHERE DeliveryOrderID = @id`);
        ...
        await logAction(req, 'UPDATE', 'billing-invoices', qid, 
            `แก้ไขใบเสนอราคา ${deliveryOrderNo || qid} — ลูกค้า: ${customerName} — ยอดรวม: ${grandTotal}`);

        res.json({ success: true, message: 'DeliveryOrder updated successfully' });

    } catch (err) {
        if (transaction) await transaction.rollback();
        console.error('Error updating deliveryOrder:', err);
        res.status(500).json({ success: false, message: 'Failed to update deliveryOrder: ' + err.message, error: err.message });
    }
});
...
router.patch('/:id/status', authorizeRoles('admin', 'sales'), validate(updateStatusSchema), async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query(`UPDATE DeliveryOrder SET Status = @status, UpdatedAt = GETDATE() WHERE DeliveryOrderID = @id`);
        ...
        console.error('Error updating deliveryOrder status:', err);
        res.status(500).json({ success: false, message: 'Failed to update status', error: err.message });
    }
});

// 6. Delete DeliveryOrder
router.delete('/:id', authorizeRoles('admin', 'sales'), async (req, res) => {
    try {
        ...
        const existing = await pool.request()
            .input('id', sql.Int, req.params.id)
            .query('SELECT DeliveryOrderNo, CustomerName, GrandTotal FROM DeliveryOrder WHERE DeliveryOrderID = @id');
        const deleted = existing.recordset[0];

        await pool.request()
            .input('id', sql.Int, req.params.id)
            .query('DELETE FROM DeliveryOrder WHERE DeliveryOrderID = @id');

        // ✅ Audit Log: ลบใบเสนอราคา (เก็บข้อมูลที่ถูกลบไว้ด้วย)
        await logAction(req, 'DELETE', 'billing-invoices', req.params.id, 
            `ลบใบเสนอราคา ${deleted?.DeliveryOrderNo || req.params.id} — ลูกค้า: ${deleted?.CustomerName}`,
            deleted, null);

        res.json({ success: true, message: 'DeliveryOrder deleted successfully' });
    } catch (err) {
        console.error('Error deleting deliveryOrder:', err);
        res.status(500).json({ success: false, message: 'Failed to delete deliveryOrder', error: err.message });
    }
});

// 7. Get History List for a DeliveryOrder
router.get('/:id/history', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            .input('id', sql.Int, req.params.id)
            .query(`
                SELECT HistoryID, Revision, Status, BackupAt as ArchivedAt, GrandTotal
        ...
        console.error('Error fetching deliveryOrder history:', err);
...
router.get('/history/:historyId', async (req, res) => {
    try {
        ...
        const headerResult = await pool.request()
            ...
            .query(`SELECT * FROM DeliveryOrderHistory WHERE HistoryID = @hid`);
        ...
        const itemsResult = await pool.request()
            .input('hid', sql.Int, req.params.historyId)
            .query(`SELECT * FROM DeliveryOrderItemHistory WHERE HistoryID = @hid ORDER BY ItemOrder ASC`);
